...
async function loadReports() {
  ...
  // جلب الفواتير العادية ومدفوعات بوابة الطلاب معًا.
  const [snapshot, portalPaymentsSnapshot, portalRegistrationsSnapshot, usersSnapshot] = await Promise.all([
    getDocs(collection(db, "invoices")),
    getDocs(collection(db, "studentPortalPayments")),
    getDocs(collection(db, "studentRegistrations")),
    getDocs(collection(db, "users"))
  ]);
  const portalRegistrations = new Map(portalRegistrationsSnapshot.docs.map(docSnap => [docSnap.id, docSnap.data()]));
  const userNames = new Map(usersSnapshot.docs.map(docSnap => {
    const user = docSnap.data();
    return [docSnap.id, user.username || user.fullname || user.email || docSnap.id];
  }));
  const portalReportInvoices = [];
  const recordedPortalRegistrations = new Set();
  const reportTypes = ['كورسات', 'ملازم', 'محاضرات فردية', 'ملازم فردية'];

  const addPortalReportInvoice = (payment, registration, date) => {
    const subscriptions = Array.isArray(registration?.selectedSubscriptions) ? registration.selectedSubscriptions : [];
    const validSubscriptions = subscriptions.filter(item => reportTypes.includes(item.type));
    const recipient = userNames.get(payment.agentId || registration?.agentId) || payment.agentId || registration?.agentId || '-';
    portalReportInvoices.push({
      id: payment.id || registration?.id || `portal-${date.getTime()}`,
      student: payment.studentName || registration?.studentName || '-',
      recipient,
      grade: registration?.grade || '-',
      studyType: registration?.studyType || '-',
      subType: validSubscriptions.map(item => item.type).filter((type, index, list) => list.indexOf(type) === index).join(' + ') || 'بيع من الموقع',
      items: validSubscriptions.length ? validSubscriptions : [{ name: 'بيع من الموقع', price: Number(payment.amount || registration?.paidAmount || 0) }],
      date,
  ...
  const addPortalMetrics = (payment, registration, date) => {
    if (!filterInvoiceByDate({ date }, period, dateVal, monthVal, yearVal)) return;
    const subscriptions = Array.isArray(registration?.selectedSubscriptions) ? registration.selectedSubscriptions : [];
    const validSubscriptions = subscriptions.filter(item => reportTypes.includes(item.type));
    const grade = normalizeGrade(registration?.grade);
    const types = [...new Set(validSubscriptions.map(item => item.type))];
    const paymentAmount = Number(payment.amount || registration?.paidAmount || 0);
    types.forEach(type => {
      const typeAmount = validSubscriptions.filter(item => item.type === type).reduce((sum, item) => sum + Number(item.price || 0), 0);
      const amount = typeAmount || (types.length === 1 ? paymentAmount : 0);
      byType[type] += amount;
      if (byGrade[grade] && byInvoiceCount[grade]) {
        byGrade[grade][type] = (byGrade[grade][type] || 0) + amount;
        byInvoiceCount[grade][type] = (byInvoiceCount[grade][type] || 0) + 1;
      }
      if (byStudyType[grade] && (registration?.studyType === 'انتظام' || registration?.studyType === 'انتساب')) {
        byStudyType[grade][registration.studyType]++;
      }
    });
    if (!types.length) byType['كورسات'] += paymentAmount;
  };

  portalPaymentsSnapshot.forEach(docSnap => {
    ...
    addPortalMetrics(payment, registration, paymentDate);
    if (filterInvoiceByDate({ date: paymentDate }, period, dateVal, monthVal, yearVal)) addPortalReportInvoice(payment, registration, paymentDate);
  });

  portalRegistrationsSnapshot.forEach(docSnap => {
    const registration = { id: docSnap.id, ...docSnap.data() };
    if (recordedPortalRegistrations.has(registration.id) || registration.paymentStatus !== 'paid') return;
    ...
    const payment = { id: registration.id, agentId: registration.agentId, studentName: registration.studentName, amount: registration.paidAmount };
    addPortalMetrics(payment, registration, paidDate);
    if (filterInvoiceByDate({ date: paidDate }, period, dateVal, monthVal, yearVal)) addPortalReportInvoice(payment, registration, paidDate);
  });
  ...
  if (lineCtx) {
    ...
    portalReportInvoices.forEach(inv => {
      const d = inv.date instanceof Date ? inv.date : new Date(inv.date);
      if (d.getFullYear() === year) {
        monthlyTotals[d.getMonth()] += (inv.items || []).reduce((sum, item) => sum + (Number(item.price) || 0), 0);
      }
    });
  ...
  portalReportInvoices.forEach(inv => {
    allInvoices.push(inv);
    invoiceCount++;
    totalIncome += (inv.items || []).reduce((sum, item) => sum + (Number(item.price) || 0), 0);
  });
  ...
  if (typeof window.renderUserSummaryTable === 'function') {
    window.renderUserSummaryTable();
  }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const userDateInput = document.getElementById('summary-day');
  const userMonthInput = document.getElementById('summary-month');
  const refreshSummaryBtn = document.getElementById('refresh-summary-btn');
  if (userDateInput) userDateInput.addEventListener('change', () => window.renderUserSummaryTable?.());
  if (userMonthInput) userMonthInput.addEventListener('change', () => window.renderUserSummaryTable?.());
  if (refreshSummaryBtn) refreshSummaryBtn.addEventListener('click', () => window.renderUserSummaryTable?.());
  ...
  window.renderUserSummaryTable = function renderUserSummaryTable() {
    ...
    if (userMonthInput && userMonthInput.value) {
      const [year, month] = userMonthInput.value.split('-').map(Number);
    ...
    if (userDateInput && userDateInput.value) {
      const day = Number(userDateInput.value);
      filteredInvoices = invoices.filter(inv => {
        let d = inv.date;
        if (d && d.toDate) d = d.toDate();
        else d = new Date(d);
